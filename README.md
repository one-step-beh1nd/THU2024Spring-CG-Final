...
在`render`中产生(i, j)对应光线的时候，加入了一个随机数的扰动。这就是比较弱的抗锯齿。比较强的抗锯齿请参考smallpt。其方法是在计算(i, j)处像素颜色时，取其附近四个像素点，分别采样`spp`次取平均，再对四个像素取平均。效果更好，速度更慢，建议不要将这个在debug期间使用。

在`pathTracer`中，我们是按照`Material->type`的三个分量，作为该材质在三种材质上的占比。这种实现实际上包含了三种材质的混合，然而我们无需实现混合。

所以如果你保证在输入文件中，`type`的三个分量互斥，那么也可以通过判断三个分量是否大于零来判断是哪种材质。
