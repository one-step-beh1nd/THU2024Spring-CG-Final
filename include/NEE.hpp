...
// NEE
            ...
            for(int i=0; i<sceneParser->getGroup()->getE_num(); i++){
                // 遍历所有的发光项
                Object3D * obj = sceneParser->getGroup()->getE_object()[i];
                Vector3f E_point, E_normal;
                obj->getPoint(E_point, E_normal);
                Vector3f L = E_point - S;            // 交点到光源的随机方向
                float dist = L.length();             // 交点到光源的距离
                L.normalize();
                Hit tmphit;
                Ray tmpray(S, L);
                if((sceneParser->getGroup()->intersect(tmpray, tmphit, 1e-3)) && fabs(tmphit.getT() - dist) < 1e-3){
                    float cos1 = abs(Vector3f::dot(tmphit.getNormal(), L));      // 交点法向与光线的夹角
                    float cos2 = abs(Vector3f::dot(E_normal, L));                // 光源点法向与光线的夹角
                    Vector3f CF = hit_color;
                    if(hit.getMaterial()->getGlossy()){
                        CF = glossy(tmpray, ray, tmphit) * (1.0 / P_RR);
                    }else{
                        CF = hit_color;
                    }
                    e += CF * tmphit.getMaterial()->getEmission() * cos1 * cos2 / (dist * dist) * obj->getArea() / PI;
                }
            }

            Vector3f CF = hit_color;
            if (hit.getMaterial()->getGlossy()) {
                CF = glossy(newRay, ray, hit) * (1.0 / P_RR);
            } else {
                CF = hit_color;
            }
            return emission * E + e + CF * pathTracer(newRay, depth+1, 0);
