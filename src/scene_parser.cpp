...
void SceneParser::parseDepthCamera()
{
    ...
    getToken(token);
    assert(!strcmp(token, "}"));
    camera = new DepthCamera(center, direction, up, width, height, angle_radians, aperture, focalLength);
}
// ====================================================================
// ====================================================================
...
Material *SceneParser::parseMaterial() {
    ...
    filename[0] = 0;
    Vector3f diffuseColor(1, 1, 1), specularColor(0, 0, 0), refractiveColor(1, 1, 1);
    float shininess = 0;
    ...
    while (true) {
        ...
        if (strcmp(token, "diffuseColor") == 0 || strcmp(token, "color") == 0) {
            ...
            getToken(filename);
        } else if (strcmp(token, "refr") == 0 || strcmp(token, "refractiveIndex") == 0)
        {
        ...
        else if (strcmp(token, "type") == 0)
        {
            ...
            refractiveColor = readVector3f();
            // std::cout <<"read refractive Color:"<< refractiveColor << std::endl;
        }
        else {
            assert (!strcmp(token, "}"));
            break;
    ...
    auto *answer = new Material(diffuseColor, specularColor, shininess, emission, type, refractiveIndex, filename, glossy);
    return answer;
}
...
Group *SceneParser::parseGroup() {
    ...
    while (num_objects > count) {
        ...
        if (!strcmp(token, "MaterialIndex")) {
            ...
            // std::cout << "parse object:" << count << std::endl;
            ...
            if(current_material->getEmission() != Vector3f::ZERO){
                answer->addE_object(ecount, object);
                ecount++;
            }

            // std::cout << "parse object end:" << count << std::endl;
        }
    }
    getToken(token);
    assert (!strcmp(token, "}"));
...
Sphere *SceneParser::parseSphere() {
    ...
    return new Sphere(center, radius, current_material, speed);
}
